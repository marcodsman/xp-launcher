...
| Beats of Rage (OpenBOR) | ✅ | ✅* | **Added + fixed 2026-07-09.** OpenBOR (2011 SDL 1.2 build — XP-safe, PE32) at `C:\XP_Share\openbor` running freeware `BOR.PAK`. **Fullscreen verified on-box** and pad fixed by **reverse-engineering the binary `Saves\BOR.cfg`** (see memory `openbor-borcfg-format`): default had P1 on keyboard + joystick disabled — patched `usejoy=1`, P1→joy0, P2→joy1 (co-op; Twin USB shows as two pads), `fullscreen=1`. Config repo-tracked at `config/openbor/`. *Pad button-feel = user couch confirm; if the 320×240 image looks small in fullscreen, raise in-game Screen Size. Engine runs a big free `.pak` library → future games get their own entries. |
